...
router.post(
  ...
  async (request, response) => {
    ...
    const model = await Model.findOne({ spreadsheetId });
    ...
    try {
      ...
      response.status(200).send({ functionErrors });
    } catch (e) {
      console.error(e);
      throw new HttpError(400, 'Bad request');
    }
...
const updateFileAndProject = async (
  spreadsheetId: string,
  file: ArrayBufferLike,
) => {
  ...
  fs.writeFile(originalFilePath, Buffer.from(file));
  ...
  const foundProject = await Project.findOne({ spreadsheetId });
  if (foundProject) {
    await refreshProject(foundProject);
  }
};
