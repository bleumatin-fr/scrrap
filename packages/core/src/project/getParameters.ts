...
const addSectors = (
  ...
  newSectorsIndex.forEach((sectorIndex) => {
    ...
    let color = '#ff0000';
    let icon: string = 'identity';
    try {
      switch (sectorIndex) {
        case 0:
          color = config.sectors[colorIndex[0]].color;
          icon = config.sectors[colorIndex[0]].icon || 'identity';
          break;
        case 1:
...
export const cleanUpSectors = (sectors: Sector[], prev: string): Sector[] => {
  return sectors
    .map((sector: Sector): Sector | null => {
      const cleanedUpSubSectors = cleanUpSectors(
        sector.sectors,
